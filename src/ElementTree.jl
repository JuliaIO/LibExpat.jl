...
# ElementTree.jl is based on ElementTree.py
...
type TreeBuilder
    ...
    TreeBuilder() = new([], [], nothing, nothing, Element)


end

function close(self::TreeBuilder) 
    if len(self.m_elem) != 0 error("missing end tags") end
    if self.m_last != nothing error("missing toplevel element") end
    self.m_last
end

function flush(self::TreeBuilder):
    if len(self.m_data) > 0
        if self.m_last != nothing
            text = "".join(self.m_data)
            if self.m_tail != nothing
...
- add data as a function to TreeBuilder

function XML_DefaultHandler (userData::Ptr{Void}, s::Ptr{Uint8}, len::Cint)
    etree = unsafe_pointer_to_objref(userData)
    
    text = bytestring(s, len)
    prefix = text[:1]
        
    if prefix == '&':
        # deal with undefined entities
        try:
            etree.target.data(etree.entity[text[1:-1]])
...
function init(p::ETreeParser, html=0, target=nothing, encoding=0):
        p.parser = LibExpat.ParserCreate(encoding)
        if (target == nothing) 
            p.target = TreeBuilder() 
        else
            p.target = target
        end
